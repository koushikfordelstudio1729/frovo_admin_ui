...
const CreateGRN = () => {
  ...
  const updateQuantity = (sku: string, field: keyof GRNQuantityItem, value: string | number) => {
    setQuantities(prev => ({
      ...prev,
      [sku]: {
        ...prev[sku],
  ...
  const validateForm = () => {
    ...
    if (challanDocument && !allowedTypes.includes(challanDocument.type)) {
      setValidationError("File type not supported. Please upload PDF, JPG, PNG, DOC, DOCX, XLS, or XLSX");
      return false;
    }

    // Validate quantities (optional for now, can be removed if backend doesn't support it yet)
    for (const [sku, qty] of Object.entries(quantities)) {
      ...
      if (qty.accepted_quantity + qty.rejected_quantity !== qty.received_quantity) {
        setValidationError(`For SKU ${sku}: Accepted + Rejected must equal Received quantity`);
        return false;
      }
      if (!qty.expiry_date) {
  ...
  const handleSubmit = async () => {
    ...
    // Optional fields
    if (formData.recieved_date) {
      formDataPayload.append('received_date', new Date(formData.recieved_date).toISOString());
    }
    if (formData.remarks) {
      formDataPayload.append('remarks', formData.remarks);
    }

    // Quantities - send as JSON string (backend parses it automatically)
    if (Object.keys(quantities).length > 0) {
      const quantitiesArray = Object.values(quantities).map(qty => ({
        ...
        // Note: expiry_date and item_remarks are collected in UI but not sent to backend yet
        // Uncomment below when backend adds support:
        // expiry_date: new Date(qty.expiry_date).toISOString(),
      ...
      <div className="max-w-full bg-white rounded-2xl p-8">
        ...
        {selectedPO && (
          <div className="mb-8 bg-blue-50 rounded-lg p-6">
            ...
            <div className="mt-6">
              <h4 className="text-md font-semibold text-gray-900 mb-3">Line Items & Quantities</h4>
                ...
                <table className="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Accepted *</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {selectedPO.po_line_items.map((item) => (
                      <tr key={item._id}>
                        <td className="px-3 py-2 text-sm text-gray-900 font-medium">{item.sku}</td>
                        <td className="px-3 py-2 text-sm text-gray-900">{item.productName}</td>
                        <td className="px-3 py-2 text-sm text-gray-600">{item.quantity} {item.uom}</td>
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(e) => updateQuantity(item.sku, 'received_quantity', parseInt(e.target.value) || 0)}
                          />
                        </td>
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(e) => updateQuantity(item.sku, 'accepted_quantity', parseInt(e.target.value) || 0)}
                          />
                        </td>
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(e) => updateQuantity(item.sku, 'rejected_quantity', parseInt(e.target.value) || 0)}
                          />
                        </td>
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(e) => updateQuantity(item.sku, 'expiry_date', e.target.value)}
                          />
                        </td>
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={(e) => updateQuantity(item.sku, 'item_remarks', e.target.value)}
                          />
                        </td>
                      </tr>
                    ))}
              ...
              <p className="text-xs text-gray-500 mt-2">* Note: Accepted + Rejected must equal Received quantity</p>
            </div>
          </div>
        )}
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    setChallanDocument(file);
                  }
                }}
                className="block w-full px-6 py-4 text-base text-gray-700 bg-white border-2 border-orange-300 rounded-lg cursor-pointer focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-orange-500 file:text-white file:cursor-pointer hover:file:bg-orange-600"
              />
            </div>
            {challanDocument && (
              <p className="mt-2 text-sm text-green-600 font-medium">
                Selected: {challanDocument.name} ({(challanDocument.size / 1024).toFixed(2)} KB)
              </p>
            )}
