...
export const RouteManagementHeader: React.FC = () => {
  const pathname = usePathname();

  function getCurrentPageLabel(items: RouteManagementMenuItem[]) {
    const found = items.find((item) => item.href === pathname);
    return found ? found.label : null;
  }

  const title =
    getCurrentPageLabel(routeManagementNavigation) || "Route Management";
