    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
      ...
      <div className="relative bg-white rounded-lg shadow-xl max-w-5xl w-full mx-4 my-8 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Product Images */}
          {purchaseOrder.po_line_items.some(item => item.images && item.images.length > 0) && (
            <div>
                  ...
                  item.images && item.images.length > 0 && (
                    <div key={item._id} className="bg-gray-50 rounded-lg p-4">
                      <h4 className="text-sm font-semibold text-gray-700 mb-3">
                        Line {item.line_no}: {item.productName} ({item.images.length} {item.images.length === 1 ? 'image' : 'images'})
                      </h4>
                          ...
                          >
                            {image.mime_type.startsWith('image/') ? (
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="block w-full h-32 flex items-center justify-center"
                              >
                            ...
                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <p className="text-xs text-white truncate font-medium">{image.file_name}</p>
                              <p className="text-xs text-gray-200">{(image.file_size / 1024).toFixed(1)} KB</p>
                            </div>
